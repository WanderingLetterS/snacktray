#define spritelist
stand,wait,lookup,pose,crouch,knock,dead,walk,run,brake,spring,springfall,jump,bonk,ball,spindash,push,hang,dash,mariojump,mariobonk,mariofall,wall,pound,slide,twirl,fire,flagslide,climbing,grind,piping,pipingup,sidepiping,doorenter,doorexit
...
sprite="climbing" frspd=sign(left+right+up+down)
        ...
        if (fall||mariojump||fakemariojump) {
            if (mariojump||fakemariojump){ 
                ...
                sprite=fallspr 
                if sprite="walk" or sprite="run" sprite="springfall"
            }
...
run=1 runvar=1
canstopjump=1
} else if (!crouch && !pound && !double && !spinjump && !carry && !mariojump) {jumpspd=0 playsfx('somarijump2',0,1+(size==5)/3) firedash=0 if size<3 hsp=hsp/2 prejumphsp=abs(hsp) vsp=-3 double=1 mariojump=1 shoot(x,y+8,psmoke,0,-1)}
