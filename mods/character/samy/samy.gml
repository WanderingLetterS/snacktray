...
#define soundlist
release,skid,spin,spindash,insta,dash,boom,firedash,slam,tatsu,upper,firekick
...
[a](Air):Insta shield. (For now.)
[b]:Hammer Tornado, Hit enemies to increase speed!
[c]:Dive
...
switch type{
    ...
    if (event="draw") {
        draw_sprite_part(owner.sheets[2],0,44+floor(frame)*17,88,16,16,round(x-7),round(y-7))
    }
    break
    case "amykaboom":
    if (!created) {
    ...
    if (event="draw") {
        draw_sprite_part_ext(owner.sheets[2],0,10+floor(frame)*17,88,16,16,round(x-8*xsc),round(y-8),xsc,1,$ffffff,1)
    }
    break
}
...
else if (tornado) {sprite="twirl" frspd=0.1+bkey*0.25+(tornadoeffect*tornado/80)*0.55}
else if (jump) {
if (onvine) 
{
...
if ((abut || jumpbufferdo) && (!springin) || (tornado && akey)) {
...
if (bkey){
	if tornado{
		tornado=max(0,tornado-0.45)
		if vsp>-2
		vsp-=0.3
		jump=1
		trip=0
	}else if !trip tornadoeffect=0
} else {if tornado>1 tornado=max(0,tornado-(0.15+0.15*!jump)) else {tornadoeffect-=0.25 tornado=(tornadoeffect/4)} if tornado && jump {vsp-=0.05} else if !trip tornadoeffect=0}
...
if (!jump) {
	...
	tornadoed=false
	if (loose || spin || crouch || piko || trip && !jump) {
		braking=0
...
maxspd=(3 + !!size*0.25 + spin + (boostvar) + firedash/24+ water*0.1+ min(tornado/50*tornadoeffect,4))*wf
if (abs(hsp)>maxspd && !trip) hsp=(abs(hsp)*2+maxspd)/3*sign(hsp)
...
sprite_angle=0
if tornado sprite_angle=-(hsp+h)*4
...
if floor(frame) ==0 && tornado{
	i=shoot(x,y+10,psmoke) i.depth=depth+2 i.hspeed=-hsp*0.5 i.vspeed=-abs(hsp)*0.5
}
...
if (coll) {
    if (!coll.damage_player_on_contact) {
		if tornado{tornado+=60 if tornado>80 tornado=80 tornadoeffect+=0.75 playsfx("samyupper")} 
        calcfall=fall
...
//hit blocks sideways
if (tornado || (spin && abs(hsp)>0.5) || (super && fall=10)) {
    global.coll=id
