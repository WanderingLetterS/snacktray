#define spritelist
stand,wait,lookup,pose,crouch,knock,dead,walk,run,brake,jump,bonk,fall,tongue,tongueup,tongueharmed,tongueupharmed,swallow,standhold,lookuphold,crouchhold,walkhold,runhold,brakehold,jumphold,bonkhold,fallhold,pound,fire,flutter,swim,paddle,twirl,fire,flutter,piping,pipingup,pipeside,dooorenter,doorexit,climbing,flagslide
...
#define effectsbehind
    ...
    {
    if aimdirgo=0
    aimdir=approach_val(aimdir,90,3)
    else
    if !(up && aimdir=90)
    aimdir=approach_val(aimdir,-45,3)
    if aimdir=90 && !up
    aimdirgo=1
    if aimdir=-45
    aimdirgo=0
...
if !up
{
coll=instance_place(x+3*xsc,y,collider)
if (c>=56) || (c>56) mode=1
...
if !owner.anemyeat
if coll && capture=0 && coll.object_index!=piranha && coll.object_index!=firepiranha
{
capture=-1
...
if tonge
{
draw_omitext(owner.x,owner.y-16,string(c))
getregion(x)
if brickpull 
{
if (tile) tile_delete(tile)
if !up
tile=tile_dyn(global.master[biome],152,264,16,16,round(owner.x)+xx*0.8*xsc,round(owner.y-2),depth+2)
else
tile=tile_dyn(global.master[biome],152,264,16,16,round(owner.x-8),round(owner.y-12-xx),depth+2)
}

xx=median(0,c,56) 
if !up
draw_sprite_part_ext(sheet,0,133+1-xx,102,xx,20,round(owner.x)+4*xsc,round(owner.y-8),xsc,1,$ffffff,1)
else
draw_sprite_part_ext(sheet,0,135,66,20,xx,round(owner.x-10*xsc),round(owner.y-12-xx),xsc,1,$ffffff,1)
...
if ((abut || jumpbufferdo) && (!springin)) {
    if (!jump||vinegrab || toungecling=2) { //jump
...
if (!fluttered && !flutter && !pound && !fulbuff) {
  vsp=0.5
  flutter=1
}
...
if holdind=shell
with instance_create(x+12*xsc+hsp,y,shell) {enemy2=other.enemy2store stop=0 spd=max(3,abs(other.hsp)+1) coll.phase=other.id owner=other.id kicked=1 type=other.storedtype other.anemyeat=id vspeed=-1 hspeed=other.hsp+4*other.xsc}
if holdind!=shell && holdind!=koopa && holdind!=redkoopa && holdind!=yelkoopa && holdind!=blukoopa && holdind!=redhover && holdind!=yelhover && holdind!=hopkoopa && holdind!=brick
with instance_create(x+12*xsc+hsp,y,holdind) {if other.bratstore {brat=1 sprite="goombrat"} enemy2=other.enemy2store owner=other.id other.anemyeat=id spitted=1 if object_index!=bulletbill && object_index!=cannonball && object_index!=redcannonball vspeed=-1 else xsc=sign(hspeed) hspeed=other.hsp+4*other.xsc}
}
if holdind=brick
...
maxspd=(3+water+slipnslide-(flutter>8)+!!spin)*wf
...
else if (flutter>8) if size=3 vsp=median(-3,vsp-0.3,2) else vsp=median(-0.9,vsp-0.3,2)
else if (water) vsp=min(1.5,vsp+0.04)
else if toungecling=2 vsp=0
else vsp=min(4,vsp+0.28)
