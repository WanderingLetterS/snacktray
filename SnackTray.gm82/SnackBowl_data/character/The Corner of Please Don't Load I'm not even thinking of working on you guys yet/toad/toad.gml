#define spritelist
stand,wait,lookup,pose,crouch,knock,dead,walk,run,brake,jump,bonk,fall,fly,wall,pound,slide,standcarry,crouchcarry,walkcarry,jumpcarry,bonkcarry,fallcarry,throw,spin,pound,swim,paddle,twirl,fire,kick,climbing,flagslide,grind,piping,pipingup,sidepiping,doorenter,doorexit


#define soundlist
skid,swim,pound,stomp,smalljump,flip,spin,slide,kick,fireball,wallkick,smallwallkick
...
else if (crouch) {sprite="crouch" if (hijumpcharge<60) {frspd=0} else if frame<1 {prevent_spr_reset=1 frame=1} }
else if (jump) {
...
sprite="climbing" frspd=sign(left+right+up+down)
	...
	else if (spinjump or nsmbtwirl) {sprite="twirl"}
	...
	if (crouch) {sprite="crouch" if (hijumpcharge<60) {frspd=0} else if frame<1 {prevent_spr_reset=1 frame=1} }
...
if (water) {if (!pound) {vsp=-1.5-up*0.75 swim=24 crouch=0 playsfx("toadswim") double=0
if (sprite="paddle") frame=0}}
else {
if (size==0) jumpsnd=playsfx("toadsmalljump")
else jumpsnd=playsfx("toadjump")
grabflagpole=0
latchedtoflagpole=0
onvine=0
if (spin) {vsp=-1 instance_create(x,y+12,smoke)}
else {vsp=-((hijumpcharge>60)*2+4.7+min(1,abs(hsp)/8)+!!poundjump)}
...
xsc*=-1 vsp=-4
if (size) playsfx("toadwallkick") else playsfx("toadsmallwallkick")
...
if (carry) {
updatecarry()
if (!down) {throw=16 instance_create(carryid.x,carryid.y-18,kickpart) sound("enemykick")}
with (carryid) {event_user(0)  gravity=0.2 }
carryid=noone
carry=0
}
}
...
maxspd=(2+runvar+water+slipnslide+(energy=maxe)*0.5-!!carry)*wf
...
if fall!=69
    ...
    if (jump) {
		...
		else if (fall!=69) vsp=min(4,vsp+0.25+(!!nsmbtwirl*-0.15))
		
		if (hang>0 && vsp>1 && !spinjump && !water) vsp=1.5
		if (run && !bkey && abs(hsp)<1) run=0
		if (skidding) {soundstop("toadskid") skidding=0}
		vine_climbing()
		braking=0
...
if (!jump) {
	...
	if (push=0 && hsp!=0 && braking) {
		if (!skidding) {skidding=1 playsfx("toadskid",1)}
	} else if (skidding) {soundstop("toadskid") skidding=0}
}
...
if (coll) {
    if (!coll.damage_player_on_contact) {
        ...
        if (spin) {
            ...
            else if (type=beetle) {hsp=0 jump=1 jumpspd=0.5 enemystomp(coll) exit}
    ...
    if (bkey && !carry) {
        ...
        } else if (coll.vspeed<0 && coll.y>y+8) {
    jump=1 
    fall=1 
    vsp=-0.5 
    if (bkey && !carry) {
