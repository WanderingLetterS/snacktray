...
var fn,state,script,c,f,str,name,protecc,i,hashud,hashudmp;
...
for ({f=file_text_open_read(fn) str="" opencount=0 closecount=0 closeline=0 blockcomment=0 hashud=0};!file_text_eof(f);file_text_readln(f)) {
...
with (instance_create(0,0,compiler_dummy)) {
    ...
    execute_string(global.unlockdata[c])
    instance_destroy()
}
if (error_occurred) {
