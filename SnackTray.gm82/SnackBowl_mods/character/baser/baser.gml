...
if ((abut || jumpbufferdo) && (!springin)) {
    if (!jump||fall=69||grabflagpole||tailsgrab||(fall!=0 && size==9 && !cloverjumped && !insted)) { //jump
        if (crouch && fall!=69)          com_startspindash()
		else if !peelready {
...
maxspd=(3.25 + !!size*0.25 + (airdash)*0.5 +boost + firedash/24)*(wf+(0.5*water)+(0.5*(is_water())*water))
...
if ((!size || is_mini() || ohgoditslava) && !shielded && global.rings[p2]==0) {
