switch(global.cobjectentrypoint){
	...
	case "step":
		...
		if alarm0 alarm0-=1
		if !collected{
			frame=round((global.itemframe/4)*frmax)
			...
			frame+=0.20
				...
				x=owner.x
				y=owner.bbox_top-16
				if alarm0>2 {owner.piped=1 owner.sprite="pose"} else owner.piped=0
	...
	case "draw":
		...
		if !collected
		draw_sprite_part_ext(sheet,0,190+16*frame,7+35*off_y,16,16,x,y,1,1,c_white,1)
		else
		draw_sprite_part_ext(sheet,0,65+25*frame,7+35*off_y,24,24,x,y,1,1,c_white,1)
	...
	case "draw_hud":
		if !global.custom_collectible_hud_drawn[realtype,view_current]{
			...
			if hud_side==0
				draw_sprite_part_ext(sheet,0,8,7+35*off_y,w,27,hud_x,hud_y,1,1,c_white,1)
			else if hud_side==1
				draw_sprite_part_ext(sheet,0,8,7+35*off_y,w,27,view_wview[view_current]/2 +hud_x,hud_y,1,1,c_white,1)
			else if hud_side==2
				draw_sprite_part_ext(sheet,0,8,7+35*off_y,w,27,view_wview[view_current] -hud_x,hud_y,1,1,c_white,1)
			
			if type==2 {
				if global.collectiblecollected[realtype,view_current]>0{
					if hud_side==0
						draw_sprite_part_ext(sheet,0,8+w+1,7+35*off_y,w,27,hud_x,hud_y,1,1,c_white,1)
					else if hud_side==1
						draw_sprite_part_ext(sheet,0,8+w+1,7+35*off_y,w,27,view_wview[view_current]/2 +hud_x,hud_y,1,1,c_white,1)
					else if hud_side==2
						draw_sprite_part_ext(sheet,0,8+w+1,7+35*off_y,w,27,view_wview[view_current] -hud_x,hud_y,1,1,c_white,1)
				}
			} else {
			
					if hud_side==2{
						//draw text "reversed"
						draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 1)),hud_x-40 ,hud_y+7)
						if num>9 draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 2)),hud_x-32,hud_y+6)
						if num>99 draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 3)),hud_x-22,hud_y+5)
					} else {
						//draw text normally

						num=global.collectiblecollected[realtype,view_current]
						
						draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 1)),hud_x+22,hud_y+7)
						if num>9 draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 2)),hud_x+32,hud_y+6)
						if num>99 draw_sprite(spr_mainuitextsmall,real(string_char_at(string(num), 3)),hud_x+40,hud_y+5)
					}
	...
	case "lemon_display":
		...
		draw_sprite_part_ext(sheet,0,190,7+35*off_y,16,16,x*16,y*16,1,1,c_white,1)
	...
	case "editobjdataname":
		...
		if global.cobjectlemontype>1 {return "----"}
		else {
